...
class Conv3d_Block(nn.Module):
    def __init__(self, num_in, num_out, kernel_size=1, stride=1, g=1, padding=None, norm=None):
        ...
        self.conv = nn.Conv3d(num_in, num_out, kernel_size=kernel_size,
                              padding=padding, stride=stride, groups=g, bias=False)
...
class DilatedConv3DBlock(nn.Module):
    def __init__(self, num_in, num_out, kernel_size=(1, 1, 1), stride=1, g=1, d=(1, 1, 1), norm=None):
        ...
        self.act_fn = nn.ReLU(inplace=True)
        self.conv = nn.Conv3d(num_in, num_out, kernel_size=kernel_size,
                              padding=padding, stride=stride, groups=g, dilation=d, bias=False)
...
class MFunit(nn.Module):
    def __init__(self, num_in, num_out, g=1, stride=1, d=(1, 1), norm=None):
        ...
        self.conv1x1x1_in1 = Conv3d_Block(
            num_in, num_in//4, kernel_size=1, stride=1, norm=norm)
        self.conv1x1x1_in2 = Conv3d_Block(
            num_in//4, num_mid, kernel_size=1, stride=1, norm=norm)
        self.conv3x3x3_m1 = DilatedConv3DBlock(num_mid, num_out, kernel_size=(
            3, 3, 3), stride=stride, g=g, d=(d[0], d[0], d[0]), norm=norm)  # dilated
        self.conv3x3x3_m2 = DilatedConv3DBlock(num_out, num_out, kernel_size=(
            3, 3, 1), stride=1, g=g, d=(d[1], d[1], 1), norm=norm)
        # self.conv3x3x3_m2 = DilatedConv3DBlock(num_out,num_out,kernel_size=(1,3,3),stride=1,g=g,d=(1,d[1],d[1]),norm=norm)

        # skip connection
        if num_in != num_out or stride != 1:
            if stride == 1:
                self.conv1x1x1_shortcut = Conv3d_Block(
                    num_in, num_out, kernel_size=1, stride=1, padding=0, norm=norm)
            if stride == 2:
                # if MF block with stride=2, 2x2x2
                self.conv2x2x2_shortcut = Conv3d_Block(
                    num_in, num_out, kernel_size=2, stride=2, padding=0, norm=norm)  # params
    ...
    def __init__(self, num_in, num_out, g=1, stride=1, norm=None, dilation=None):
        ...
        for i in range(3):
            self.conv3x3x3_m1.append(
                DilatedConv3DBlock(num_mid, num_out, kernel_size=(3, 3, 3), stride=stride, g=g, d=(
                    dilation[i], dilation[i], dilation[i]), norm=norm)
            )

        # It has not Dilated operation
        self.conv3x3x3_m2 = DilatedConv3DBlock(num_out, num_out, kernel_size=(
            3, 3, 1), stride=(1, 1, 1), g=g, d=(1, 1, 1), norm=norm)
        ...
        if num_in != num_out or stride != 1:
            ...
            if stride == 2:
                self.conv2x2x2_shortcut = Conv3d_Block(
                    num_in, num_out, kernel_size=2, stride=2, padding=0, norm=norm)

    def forward(self, x):
        x1 = self.conv1x1x1_in1(x)
        x2 = self.conv1x1x1_in2(x1)
        x3 = self.weight1*self.conv3x3x3_m1[0](x2) + self.weight2*self.conv3x3x3_m1[1](
...
class MFNet(nn.Module):
    ...
    def __init__(self, c=4, n=32, channels=128, groups=16, norm='bn', num_classes=4):
        ...
        self.encoder_block1 = nn.Conv3d(
            c, n, kernel_size=3, padding=1, stride=2, bias=False)  # H//2
        ...
        self.encoder_block3 = nn.Sequential(
            MFunit(channels, channels*2, g=groups,
                   stride=2, norm=norm),  # H//8
            MFunit(channels * 2, channels * 2, g=groups, stride=1, norm=norm),
            MFunit(channels * 2, channels * 2, g=groups, stride=1, norm=norm)
        )

        self.encoder_block4 = nn.Sequential(  # H//8,channels*4
            MFunit(channels*2, channels*3, g=groups,
                   stride=2, norm=norm),  # H//16
            MFunit(channels*3, channels*3, g=groups, stride=1, norm=norm),
            MFunit(channels*3, channels*2, g=groups, stride=1, norm=norm),
        )

        self.upsample1 = nn.Upsample(
            scale_factor=2, mode='trilinear', align_corners=False)  # H//8
        self.decoder_block1 = MFunit(
            channels*2+channels*2, channels*2, g=groups, stride=1, norm=norm)

        self.upsample2 = nn.Upsample(
            scale_factor=2, mode='trilinear', align_corners=False)  # H//4
        self.decoder_block2 = MFunit(
            channels*2 + channels, channels, g=groups, stride=1, norm=norm)

        self.upsample3 = nn.Upsample(
            scale_factor=2, mode='trilinear', align_corners=False)  # H//2
        self.decoder_block3 = MFunit(
            channels + n, n, g=groups, stride=1, norm=norm)
        self.upsample4 = nn.Upsample(
            scale_factor=2, mode='trilinear', align_corners=False)  # H
        self.seg = nn.Conv3d(n, num_classes, kernel_size=1,
...
class DMFNet(MFNet):  # softmax
    ...
    def __init__(self, c=4, n=32, channels=128, groups=16, norm='bn', num_classes=4):
        ...
        self.encoder_block2 = nn.Sequential(
            DMFUnit(n, channels, g=groups, stride=2, norm=norm,
                    dilation=[1, 2, 3]),  # H//4 down
            DMFUnit(channels, channels, g=groups, stride=1,
                    norm=norm, dilation=[1, 2, 3]),  # Dilated Conv 3
            DMFUnit(channels, channels, g=groups, stride=1,
                    norm=norm, dilation=[1, 2, 3])
        )

        self.encoder_block3 = nn.Sequential(
            DMFUnit(channels, channels*2, g=groups, stride=2,
                    norm=norm, dilation=[1, 2, 3]),  # H//8
            DMFUnit(channels * 2, channels * 2, g=groups, stride=1,
                    norm=norm, dilation=[1, 2, 3]),  # Dilated Conv 3
            DMFUnit(channels * 2, channels * 2, g=groups,
                    stride=1, norm=norm, dilation=[1, 2, 3])
        )


class DMFNet_(nn.Module):
    def __init__(self, c=4, num_classes=4, n=32, channels=128, groups=16, norm='bn'):
        super(DMFNet_, self).__init__()

        # Entry flow
        self.encoder_block1 = nn.Conv3d(
            c, n, kernel_size=3, padding=1, stride=(1, 2, 2), bias=False)  # H//2
        self.encoder_block2 = nn.Sequential(
            DMFUnit(n, channels, g=groups, stride=2, norm=norm,
                    dilation=[1, 2, 3]),  # H//4 down
            DMFUnit(channels, channels, g=groups, stride=1,
        ...
        self.reconstruction_block1 = nn.Sequential(
            MFunit(channels, n, g=groups, stride=1, norm=norm),
        ...
        self.reconstruction_block2 = nn.Sequential(
            MFunit(channels * 2, channels, g=groups, stride=1, norm=norm),
            nn.Upsample(scale_factor=2, mode='trilinear',
                        align_corners=False)  # H//2
        )

        self.encoder_block4 = nn.Sequential(  # H//8,channels*4
            DMFUnit(channels*2, channels*3, g=groups, stride=2,
                    norm=norm, dilation=[1, 2, 3]),  # H//16
            DMFUnit(channels*3, channels*3, g=groups,
                    stride=1, norm=norm, dilation=[1, 2, 3]),
            DMFUnit(channels*3, channels*2, g=groups,
                    stride=1, norm=norm, dilation=[1, 2, 3]),
        )
    ...
    def forward(self, x):
        ...
        re1 = self.reconstruction_block1(x2)
        re2 = self.reconstruction_block2(x3)
...
if __name__ == '__main__':
    ...
    model = DMFNet_(c=4, num_classes=3)
    model.cuda(device)
    y, re1, re2 = model(x)
    print(y.shape, re1.shape, re2.shape)
    # import os
    # os.environ['CUDA_VISIBLE_DEVICES'] = '0'
    # device = torch.device('cuda:0')
    # x = torch.rand((1,4,128,128,128),device=device) # [bsize,channels,Height,Width,Depth]
    # a = nn.Conv3d(4, 4, kernel_size=3, padding=1, stride=(1,2,2), bias=False).cuda()# H//2
    # print(a(x).shape)
