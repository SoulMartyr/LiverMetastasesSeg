...
class DMFNet_(nn.Module):
    def __init__(self, c=4,n=32,channels=128,groups = 16,norm='bn', num_classes=4):
        ...
        # Entry flow
        self.encoder_block1 = nn.Conv3d( c, n, kernel_size=3, padding=1, stride=(1, 2, 2), bias=False)# H//2
        self.encoder_block2 = nn.Sequential(
            DMFUnit(n, channels, g=groups, stride=2, norm=norm,dilation=[1,2,3]),# H//4 down
            DMFUnit(channels, channels, g=groups, stride=1, norm=norm,dilation=[1,2,3]), # Dilated Conv 3
            DMFUnit(channels, channels, g=groups, stride=1, norm=norm,dilation=[1,2,3])
        ...
        self.reconstruction_block1 = nn.Sequential(
            ...
            nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False) # H//2
        )
        ...
        self.encoder_block4 = nn.Sequential(# H//8,channels*4
            DMFUnit(channels*2, channels*3, g=groups, stride=2, norm=norm, dilation=[1,2,3]), # H//16
            DMFUnit(channels*3, channels*3, g=groups, stride=1, norm=norm, dilation=[1,2,3]),
            DMFUnit(channels*3, channels*2, g=groups, stride=1, norm=norm, dilation=[1,2,3]),
        )
...
if __name__ == '__main__':
    ...
    model = DMFNet_(c=4, groups=16, norm='sync_bn', num_classes=4)
    model.cuda(device)
    y, re1, re2 = model(x)
