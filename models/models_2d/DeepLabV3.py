...
class _DeepLabHead(nn.Module):
    def __init__(self, in_channel, nclass, norm_layer=nn.BatchNorm2d, norm_kwargs=None, **kwargs):
        ...
        self.aspp = _ASPP(in_channel, [
                          12, 24, 36], norm_layer=norm_layer, norm_kwargs=norm_kwargs, **kwargs)
...
class _ASPP(nn.Module):
    def __init__(self, in_channels, atrous_rates, norm_layer, norm_kwargs, out_channels=256, **kwargs):
        ...
        self.b1 = _ASPPConv(in_channels, out_channels,
                            rate1, norm_layer, norm_kwargs)
        self.b2 = _ASPPConv(in_channels, out_channels,
                            rate2, norm_layer, norm_kwargs)
        self.b3 = _ASPPConv(in_channels, out_channels,
                            rate3, norm_layer, norm_kwargs)
...
if __name__ == "__main__":
    ...
    model = DeepLabV3(1, "resnet34", pretrained_base=True, dilated=False)
    out = model(img)
    print(out.size())
