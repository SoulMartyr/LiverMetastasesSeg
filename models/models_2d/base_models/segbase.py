...
from .resnetv1b import resnet50_v1b, resnet101_v1b, resnet152_v1b, resnet34_v1b
...
class SegBaseModel(nn.Module):
    ...
    def __init__(self, backbone='resnet50', pretrained_base=False, dilated=False, **kwargs):
        ...
        if backbone == "resnet34":
            ...
            self.backbone = resnet50_v1b(
                pretrained=pretrained_base, dilated=dilated, **kwargs)
            self.base_channel = [256, 512, 1024, 2048]
        elif backbone == 'resnet101':
            self.backbone = resnet101_v1b(
                pretrained=pretrained_base, dilated=dilated, **kwargs)
            self.base_channel = [256, 512, 1024, 2048]
        elif backbone == 'resnet152':
            self.backbone = resnet152_v1b(
                pretrained=pretrained_base, dilated=dilated, **kwargs)
            self.base_channel = [256, 512, 1024, 2048]
            ...
            if dilated:
                self.backbone = dilated_densenet201(
                    8, pretrained=pretrained_base)
            else:
