...
class PVTEncoderLayer(BaseModule):
    ...
    def __init__(self,
        ...
        self.ffn = MixFFN(
            ...
            act_cfg=act_cfg)
        self.global_layer_attention = global_layer_attention(embed_dims=embed_dims)
    ...
    def forward(self, x, B, T, hw_shape, msg_tokens, msg_shift=None):
        ...
        x,frame_attention = self.global_layer_attention(x,msg_tokens)
        return x, msg_tokens,frame_attention

class global_layer_attention(nn.Module):
    def __init__(self,embed_dims):
        super().__init__()
        ##后续传入参数
        hidden_dims=int(embed_dims/2)
        self.linear_1 = nn.Linear(embed_dims,hidden_dims)
        self.linear_2 = nn.Linear(hidden_dims,embed_dims)

    def forward(self, x, msg_tokens):
        ## 本质是对每一帧做聚合操作
        shortcut =  x

        msg_tokens = msg_tokens.sum(dim=1).squeeze()
        msg_tokens = self.linear_1(msg_tokens)
        msg_tokens= nn.ReLU6(inplace=True)(msg_tokens)
        msg_tokens = self.linear_2(msg_tokens).unsqueeze(0)
        msg_tokens = nn.AdaptiveAvgPool1d(1)(msg_tokens).view(1,-1) ##batchsize
        global_attention = nn.Softmax(dim=-1)(msg_tokens).squeeze()
        x = shortcut + torch.einsum('bnc,b->bnc',[x,global_attention])
    ...
    def forward(self, B, T, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            for _, block in enumerate(layer[1]):
                x, msg_tokens,frame_attention = block(x, B, T, hw_shape, msg_tokens, msg_shift=self.msg_shift[i][_])
                ## 设计msg——tokens来全局层间特征
            x, msg_tokens = layer[2](x), layer[2](msg_tokens)
            x = nlc_to_nchw(x, hw_shape)
            if i in self.out_indices:
                c,h,w=x.size()[1:]
                outs.append(x.resize(B,c,T,h,w))
                global_layer_attention.append(frame_attention.resize(B,T))

        return outs,global_layer_attention
...
# model = MsgShifT(**{'embed_dims': 64, 'num_layers': [2, 2, 2, 2], 'init_cfg': None})
# X = torch.zeros([36,3,384,640])
# model(1,36,X)
# X_TEST = torch.randn([36,15360,64])
# msg_tokens = torch.randn([36,32,64])

# global_attention_model = global_layer_attention()
# global_attention_model(X_TEST,msg_tokens)